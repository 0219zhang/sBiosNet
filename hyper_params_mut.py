...
def main():
    ...
    for param in tqdm(param_grid, total=len(list(param_grid))):
        ...
        hyper_result.to_csv(
            'hyper_params_search/onlymut_param_result.csv', index=None
        )
        if hyper_auc > best_hyper_auc:
            best_hyper_auc = hyper_auc
            best_time = end - start
            pred_df.to_csv(
                'hyper_params_search/onlymut_pred_df.csv', index=None
            )
            with open('hyper_params_search/onlymut_best_wt.pkl', 'wb') as f:
                pickle.dump(best_wt, f)
            with open('hyper_params_search/onlymut_best_params.pkl', 'wb') as f:
            ...
            if best_time > end - start:
                ...
                with open(
                    'hyper_params_search/onlymut_best_wt.pkl', 'wb'
                ) as f:
                    pickle.dump(best_wt, f)
                with open(
                    'hyper_params_search/onlymut_best_params.pkl', 'wb'
                ) as f:
                    pickle.dump(best_params, f)
